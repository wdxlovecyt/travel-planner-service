...
type ChatResult = string | RoutePlanResponse | RoutePlanBatchResponse;
...
async function callDeepSeek(messages: ChatMessage[], tools?: unknown) {
...
function toHighlights(snippet: string) {
...
function dedupePlaces<T extends { name: string }>(drafts: T[]) {
...
async function buildRoutePlan(toolOutput: string, query: string): Promise<RoutePlanBatchResponse> {
  const data = JSON.parse(toolOutput) as {
    source?: string;
    ...
    raw_search_results?: GuideSearchResult[];
  };

  const guideCandidates = (Array.isArray(data.results) ? data.results : [])
    .filter(
      (item: GuideSearchResult) =>
        item.source_type === "zhihu" || item.source_type === "mafengwo",
    )
    .slice(0, 8);
  const primaryLocation = (data.primary_location ?? "").trim();

  const fallbackCandidates = (
    Array.isArray(data.raw_search_results) ? data.raw_search_results : []
  )
    .filter((item: GuideSearchResult) => item.is_target_site === true)
    .filter(
      (item: GuideSearchResult) =>
        item.site_platform === "zhihu" || item.site_platform === "mafengwo",
    )
    .sort(
      (a: GuideSearchResult, b: GuideSearchResult) =>
  ...
  const llmCandidatesPayload = candidatesForLLM.map((item: GuideSearchResult) => {
  ...
  type LLMRoute = {
    ...
    places: Array<{ name: string; evidence?: string; day?: number }>;
  };
  ...
  const extractionMessages: ChatMessage[] = [
    {
      role: "system",
      content:
        "你是旅行规划助手。请从每条候选攻略结果中抽取该攻略真正提到的地名（城市/区/街道/景点/商圈/步行街等）。必须只输出 JSON，不要输出任何解释文本。",
    },
    {
      role: "user",
  ...
  const parsed = safeJsonParse(llmContent) as { routes?: LLMRoute[] } | null;
  ...
  const routeByIndex = new Map<number, LLMRoute>();
  for (const r of llmRoutes) {
    if (typeof r.candidate_index !== "number") continue;
    if (!Array.isArray(r.places)) continue;
    routeByIndex.set(r.candidate_index, r);
  }

  const rawSearchResults = Array.isArray(data.raw_search_results) ? data.raw_search_results : [];
  const routes: RoutePlanResponse[] = [];
  for (let idx = 0; idx < candidatesForLLM.length; idx += 1) {
    const candidate = candidatesForLLM[idx];
    if (!candidate) continue;
    const sourceType: "zhihu" | "mafengwo" =
      candidate.source_type === "zhihu" || candidate.source_type === "mafengwo"
        ? candidate.source_type
        : candidate.site_platform === "zhihu" || candidate.site_platform === "mafengwo"
          ? (candidate.site_platform as "zhihu" | "mafengwo")
          : "zhihu";
    const siteName =
      (candidate.site_name ?? "").trim() || (sourceType === "zhihu" ? "知乎" : "马蜂窝");
    ...
    const normalizedDrafts = dedupePlaces(
      rawPlaces
        .map((p, placeIndex) => ({
          name: (p.name ?? "").trim(),
          ...
          sourceType,
          day:
            typeof p.day === "number" && Number.isFinite(p.day) && p.day >= 1
              ? Math.floor(p.day)
              : undefined,
          placeIndex,
        }))
    ...
    const places: RoutePlace[] = normalizedDrafts.map((draft, pIdx) => {
      const summary = toGuideSummary(draft.snippet);
      const dayPrefix =
        typeof draft.day === "number" && draft.day >= 1 ? `Day${draft.day}:` : "";
      const normalizedSummary =
        dayPrefix && !summary.toLowerCase().includes(`day${draft.day}`)
          ? `${dayPrefix}${summary}`
          : summary;
      return {
    ...
    for (let i = 0; i < places.length - 1; i += 1) {
      ...
      segments.push({
        ...
        guide: {
          ...
          references: [candidateUrl].filter((u: string) => u.length > 0),
        },
      });
    }

    const routeMetadata: RoutePlanResponse["metadata"] = {
...
async function runGuideChatWithTool(userPrompt: string): Promise<RoutePlanBatchResponse> {
  const tools = [
    {
      type: "function",
      function: {
        ...
        description:
          "Search travel guides from Zhihu and Mafengwo for one destination. This tool REQUIRES a place_name.",
        parameters: {
          type: "object",
          properties: {
            place_name: {
              type: "string",
              description:
                "Required place name for travel guide search. Must be a geographic location such as city, district, or street (e.g. 深圳, 南山区, 东门老街).",
            },
          },
          required: ["place_name"],
        },
  ...
  const messages: ChatMessage[] = [
    {
      role: "system",
      content:
        "当用户要旅游攻略时，你必须从当前对话中提取地名并调用 search_travel_guide。该工具会检索知乎和马蜂窝，必须提供 place_name，且 place_name 必须是地名（城市/区/街道/景区名）。如果无法识别地名，不要猜测，直接返回需要补充地名。",
    },
  ...
  try {
    ...
    const args = JSON.parse(argsText) as { place_name?: unknown };
    const placeName =
...
async function runChat(userPrompt: string): Promise<ChatResult> {
...
function isGuideQuery(prompt: string) {
...
function readJsonBody(req: import("node:http").IncomingMessage): Promise<{ message?: string }> {
