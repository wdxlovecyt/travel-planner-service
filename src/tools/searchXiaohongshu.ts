...
type GuidePlatform = "zhihu" | "mafengwo";
...
type RawSearchResult = NormalizedResult & {
  ...
  site_platform: GuidePlatform | "other";
...
const STRONG_TRAVEL_INTENT_PATTERN =
  /旅游|旅行|自由行|行程|路线|景点|打卡|citywalk|游玩|玩法|周边游|出游|攻略游记/i;
const LIGHT_TRAVEL_INTENT_PATTERN = /好玩|去玩|玩耍|玩一下|哪里玩|推荐一下|值得去/i;
const GUIDE_WORD_PATTERN = /攻略/i;
const TRAVEL_CONTEXT_PATTERN =
  /旅游|旅行|行程|景点|目的地|出行|打卡|游玩|交通|住宿|酒店|门票|美食|古镇|公园|博物馆|步行街|商圈/i;
const NON_TRAVEL_NOISE_PATTERN =
  /openclaw|minimax|扫地机器人|冰箱|联系我们|技能|skill|网络底座|ai\s*agent|家电|博查搜索/i;

function buildTopicKeywords(topic: string) {
  return topic
    .toLowerCase()
    .split(/[\s,，。;；、|/\\\-]+/)
    .map((part) => part.trim())
...
function hasTravelIntent(item: NormalizedResult) {
...
function buildLocationAliases(primaryLocation: string) {
  ...
  const cityDistrictMatch = compact.match(
    /^([\u4e00-\u9fa5]{2,4})(?:市)?([\u4e00-\u9fa5]{1,16}(?:区|县|旗|镇|乡|街道|街|路|巷|村|湾|湖|山|公园|古城|古镇|步行街|商圈|景区))$/,
  );
  ...
  if (cityDistrictMatch?.[2]) {
    ...
    const districtCore = district.replace(
      /(区|县|旗|镇|乡|街道|街|路|巷|村|湾|湖|山|公园|古城|古镇|步行街|商圈|景区)$/g,
      "",
    );
...
function containsPrimaryLocation(item: NormalizedResult, primaryLocation: string) {
  if (!primaryLocation) {
    return false;
  }
...
function stripWeatherContext(query: string) {
  const stripped = query
    .replace(
      /根据天气|结合天气|按天气|看天气|天气如何|天气怎么样|天气情况|天气预报|温度|气温|湿度|降雨|下雨|晴天|阴天|多云|风力|北风|南风|东风|西风|几度|°c|℃|摄氏度/gi,
      " ",
    )
    .replace(/[，。,.!！?？:：;；]/g, " ")
    .replace(/\s+/g, " ")
...
function normalizePlaceName(rawPlace: string) {
...
function splitChineseCompositeLocation(place: string) {
...
function buildPlannedQueries(normalizedPlace: string) {
...
function platformFromUrl(url: string): GuidePlatform | null {
  try {
...
function isRelevantByTopic(item: NormalizedResult, keywords: string[]) {
...
function hasGuideFriendlyPath(item: NormalizedResult, platform: GuidePlatform) {
...
function isLowValuePage(item: NormalizedResult, platform: GuidePlatform) {
  const path = getUrlPath(item.url);
...
export async function searchXiaohongshuGuideByPlace(input: PlaceSearchInput) {
  if (!bochaApiKey) {
  ...
  const strictResults: Array<
    NormalizedResult & { platform: GuidePlatform; relevance_score: number }
  > = [];
  const weakResults: Array<
    NormalizedResult & { platform: GuidePlatform; relevance_score: number }
  > = [];
  ...
  const selected =
    selectedStrict.length > 0 ? selectedStrict : weakResults.slice(0, maxResults);
